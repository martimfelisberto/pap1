<x-kaira-layout>
@extends('Produtos.clothes.calcascalcoes')

@section('page-title')
    Calças e Calções {{ ucfirst($genero) }}
@endsection

@section('page-description')
    Explore nossa coleção de calças e calções para {{ $genero }}
@endsection
...
@section('additional-filters')
    ...
    <div>
        ...
        <select name="tipo" class="mt-1 w-full rounded-md border-gray-300">
            <option value="">Todos os tipos</option>
            <option value="calcas" {{ request('tipo') == 'calcas' ? 'selected' : '' }}>Calças</option>
            <option value="calcoes" {{ request('tipo') == 'calcoes' ? 'selected' : '' }}>Calções</option>
        </select>
    </div>
